-- cloud_presets.lua — direct port of metar-dcs-app/metar.py (CLOUD_PRESETS
-- + select_cloud_preset). Table of DCS cloud presets: name -> base range in
-- meters MSL, grouped by cover type. Source: evogelsa/DCS-real-weather
-- (weather/weather.go), validated against real DCS.
...
-- If precip is true, a rain preset is ALWAYS prioritized (even if its base
-- range fits worse) over a rain-free one that's more faithful to the real
-- base — see metar-dcs-app/metar.py for the justification.
function M.select(cover, base_m, precip)
    ...
    if precip and (cover == "OVC" or cover == "BKN" or cover == "SCT") then
        ...
        -- this cover type has no rain preset at all (e.g. FEW)
    end
